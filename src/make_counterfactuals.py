"""Lesion-masked / same-size control block / background-masked variants (306-image mask subset)."""
import pandas as pd, numpy as np, os
from PIL import Image
m = pd.read_csv("data/ham_test_manifest.csv")
m = m[m.mask_path.astype(str).str.len() > 3].head(306)
...
for _, r in m.iterrows():
    ...
    ys, xs = np.where(mk); h, w = ys.ptp() + 1, xs.ptp() + 1
    ctrl = None
    if h < im.shape[0] and w < im.shape[1]:
        for _ in range(300):
            y0 = rng.integers(0, im.shape[0] - h); x0 = rng.integers(0, im.shape[1] - w)
            if not mk[y0:y0 + h, x0:x0 + w].any():
                ctrl = im.copy(); ctrl[y0:y0 + h, x0:x0 + w] = fill; break
    if ctrl is None: dropped += 1; continue          # cannot place control -> exclude, report in paper
...
pd.DataFrame(rows).to_csv("data/cf_manifest.csv", index=False)
print("variants:", len(rows), "images:", len(rows) // 3, "dropped:", dropped)
